...
- **Smart warnings** — proactive notifications that re-fire every 30 minutes so you don't miss them:
  - **Session elevated** — notifies when session usage ≥ 60% with 1h+ until reset; escalates to warning at 75%, critical at 90%
  - **Weekly elevated** — notifies when weekly usage ≥ 50% with 24h+ until reset; escalates at 75%/90%
  - **Use it or lose it** — nudges when weekly usage < 80% with < 3 days until reset (unused capacity about to expire)
